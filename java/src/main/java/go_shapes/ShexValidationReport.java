...
public class ShexValidationReport extends ModelValidationReport{
	...
	public String getAsText() {
		...
		for(Violation violation : getViolations()) {
			report+="node: "+violation.getNode_id()+" ";
			ShexViolation sv = (ShexViolation) violation;
			for(ShexExplanation e : sv.getExplanations()) {
				report+="was expected to match shape: "+e.shape_id;
				report+=" but did not fit the following constraints:";
				for(ShexConstraint c : e.getConstraints()) {
					report+="\n\tthe objects of assertions made with "+c.property_id+" should be nodes that fit the one of these shapes: ";
					report+="\n\t\t"+c.getIntended_range_shapes(); 
					report+="\n\t\tbut, sadly, the object "+c.getTarget_node_uri()+" of one such assertion emanating from the failing node here did not.\n";
				}
			}  
		}
		return report;
